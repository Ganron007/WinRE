...
###############################################################################
# ==== WinRE patch (appended) =================================================
# Upstream content above is UNMODIFIED (https://github.com/mandiant/flare-vm,
# Apache-2.0). This appended block is WinRE-specific:
#   - retries lib-bad packages with --ignore-dependencies (fixes pinned-dep
#     cascades, e.g. vcredist140 newer than pin -> python3.vm/sysinternals.vm)
#   - ensures classic cdb (FlareVM 2026 ships Store WinDbg only; WinRE needs it)
#   - prints a WinRE tool checklist; exit code 0/1
# See install\flarevm\README.md.
###############################################################################
$__winrePostfix = Join-Path $PSScriptRoot 'flarevm-postfix.ps1'
if (Test-Path $__winrePostfix) {
    & $__winrePostfix
} else {
    Write-Host "[winre] flarevm-postfix.ps1 not found next to install.ps1 - skipping WinRE post-install repair" -ForegroundColor Yellow
}
